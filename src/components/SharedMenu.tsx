...
const SharedMenu: React.FC<SharedMenuProps> = ({ menus, open, expandedMenu, toggleDropdown }) => {
                        ...
                        <div className="flex relative items-center w-full">
                          ...
                          {menu.children && (
                            <div className="absolute right-2 top-1">
                              {expandedMenu === menu.title ? <ChevronUp /> : <ChevronDown />}
                            </div>
                          )}
                        </div>
                      </span>
              ...
              <ul className="pl-4 pt-2 space-y-2">
                {menu.children.map((child, childIndex) => (
                  <li key={childIndex}>
                    <Link
                      className={`w-full flex items-center space-x-2 hover:bg-[#6699CC] dark:hover:bg-[#292929] active:bg-gray-300 py-2.5 px-2.5 rounded-lg text-white ${
                        isActive(child.path) ? 'bg-[#30425b] border border-[#3f5777] text-white dark:bg-[#292929]' : ''
                      }`}
                      href={child.path}
                    >
                      {child.icon && <span className="mr-0.5 text-white dark:text-[#888888]">{child.icon}</span>}
                      <span className={`${!open && 'hidden'} origin-left duration-200 flex items-center w-full`}>
                        {child.title}
                      </span>
                    </Link>
